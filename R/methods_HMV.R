.hmvPortfolio <- function(mu, Sigma, control = list()) {
  ...
  for (i in 1:nrow(S)) {
    ...
    if (is.na(w[i])) {
      ...
      if (round(sum(LBsub), 6) == 1) {
        ...
        Amat_Global <<- Amat
        bvec_Global <<- bvec
        gamma_Global <<- ctr$gamma
        w[idx] <- quadprog::solve.QP(Dmat = ctr$gamma[1] * as.matrix(cov), dvec = mu[idx], 
